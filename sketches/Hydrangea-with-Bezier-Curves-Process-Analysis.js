...
let speed = 3; // テキストが流れる速度
let descriptionText =
  "これはプログラムによるアジサイの素描である。この素描はプログラムによる形の生成と決定の方法に焦点をあてている。その方法を示すために、座標や回転の情報がこのスケッチに反映されている。キャンバスの中央（width / 2, height / 2）を基準にシェイプ（1つの花）を描画し、各シェイプはランダムな角度で回転する。シェイプは4つのベジェ曲線のセグメント（1つの花びら）で構成され、それぞれのセグメントは90度ずつ回転しながら描かれる。ベジェ曲線は、始点（SP）から終点（EP）に向かう滑らかな曲線であり、2つの制御点（CP1、CP2）によって引っ張られることで形が決まる。制御点は曲線の形状を引っ張ってコントロールし、これにより、曲線の滑らかさや曲がり具合が調整される。曲線の制御点や終点の位置はランダムに変動し、それが形のバリエーションを生み出す。ベジェ曲線はbeginShape()とbezierVertex()で描画され、各セグメントには次のポイントが含まれる。SP (Start Point): 始点、CP1 (Control Point 1): 曲線の最初の制御点、CP2 (Control Point 2): 曲線の2番目の制御点、EP (End Point): 終点、CP3 (Control Point 3): 終点から始点に戻る際に曲線を引っ張る制御点、CP4 (Control Point 4): 始点に戻る際に曲線を引っ張る制御点。ベジェ曲線はSPから始まり、CP1、CP2に引っ張られてEPまで滑らかな曲線を描く。その後、CP3、CP4に引っ張られながら再びSPに戻り、1つのセグメント（1つの花びら）が完了する。4回繰り返してシェイプ全体（1つの花）を形成する。さらに、random()を使用して各制御点や終点の座標にランダムな変動を加えるため、シェイプの形は確率的に決定され、毎回異なる形となる。";
...
function setup() {
  ...
  testText.style("z-index", "9999"); // Ensure it stays on top
